...
An online Wiselib-based source compilation infrastructure for the cloud. This is a fork of [codebender.cc](www.codebender.cc). The [Wiselib](www.wiselib.org) framework is difficult and time-consuming in installation, given the fact that we have so many different platforms, and installation + configuration for each platform is (i) _different_, (ii) _difficult_, and (iii) _time consuming_. Many developers prefer using preinstalled Wisebender platform based virtual machine. This may not be the quickest and easiest way of developing applications based on Wisebender. Hence, this project aims at easing the aforementioned problems by providing user with cloud infrastructure to write/import/compile Wiselib-based source code on the fly.
